...
def test_creates_object():
    diary_entry = DiaryEntry(EXAMPLE_TITLE, EXAMPLE_CONTENT)
...
def test_count_words_in_long_entry():
    diary_entry = DiaryEntry(EXAMPLE_TITLE, ((EXAMPLE_CONTENT + " ") * 100))
...
def test_read_first_10_words():
    content_long = (EXAMPLE_CONTENT + " ") * 10
    diary_entry = DiaryEntry(EXAMPLE_TITLE, content_long)
    assert diary_entry.reading_chunk(10, 1) == ((EXAMPLE_CONTENT + " ") * 5).rstrip()
...
def test_read_first_chunk_then_second():
    ...
    diary_entry = DiaryEntry(EXAMPLE_TITLE, content_one + " " + content_two)
